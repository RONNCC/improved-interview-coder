...
export class ConfigHelper extends EventEmitter {
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    // Models from SettingsDialog.tsx (see file_context_0)
    ...
    const defaultModels: Record<"openai" | "gemini" | "anthropic", string> = {
      openai: "gpt-4.1",
    ...
    if (!allowedModels.includes(model)) {
      console.warn(
        `Invalid ${provider.charAt(0).toUpperCase() + provider.slice(1)} model specified: ${model}. Using default model: ${defaultModel}`
      );
      return defaultModel;
    }
    return model;
